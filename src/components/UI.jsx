...
export function Badge({ children }) {
  return (
    <span style={{
      display: "inline-block", padding: "6px 14px", fontSize: 12,
      fontFamily: F.sans, fontWeight: 600, textTransform: "uppercase",
      letterSpacing: "0.14em", color: C.accentDeep,
      background: C.mist, border: `1px solid ${C.accent}33`, borderRadius: 2,
    }}>{children}</span>
  );
}
...
export function SectionLabel({ label, number }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 40 }}>
      <span style={{
        fontFamily: F.mono, fontSize: 12, color: C.accentDeep,
        letterSpacing: "0.06em", fontWeight: 500,
      }}>{number}</span>
      <div style={{ height: 1, width: 40, background: C.rule }} />
      <span style={{
        fontFamily: F.sans, fontSize: 12, color: C.accentDeep, fontWeight: 600,
  ...
  return (
    ...
    >
      ...
      <h3 style={{
        ...
        color: C.dark, margin: "0 0 16px 0", letterSpacing: "-0.005em",
      }}>{title}</h3>
      <p style={{
        fontFamily: F.sans, fontSize: 15, color: C.gray,
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, flex: 1 }}>
        {features.map((f, i) => (
          <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
            ...
            <span style={{ fontFamily: F.sans, fontSize: 14, color: C.grayLight, lineHeight: 1.6 }}>{f}</span>
          </div>
        ))}
      </div>
      <Link
...
export function PainCard({ number, text }) {
  return (
    <div style={{
      ...
      <span style={{ fontFamily: F.sans, fontSize: 15, color: C.grayLight, lineHeight: 1.65 }}>{text}</span>
    </div>
  );
}
